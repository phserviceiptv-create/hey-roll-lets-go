...
export default function Storefront() {
  ...
  return (
    <main className="min-h-screen bg-[#050505] text-white">
      ...
      <section className="border-t border-[#d71920]/30 bg-[#0a0a0a] px-5 py-16 lg:px-8">
        <div className="mx-auto max-w-[900px] text-center">
          <p className="text-xs font-black uppercase tracking-[.3em] text-[#d71920]">Seu pedido está quase pronto</p>
          <h2 className="rock-title mt-3 text-4xl sm:text-5xl">JÁ ESCOLHEU SEUS HITS?</h2>
          <p className="mx-auto mt-4 max-w-2xl text-white/60">Confira o carrinho e finalize somente quando terminar de escolher tudo o que deseja.</p>
          <div className="mt-7 flex flex-col items-center justify-center gap-4 sm:flex-row">
            <div className="border border-white/10 bg-black px-6 py-4 text-left">
              <div className="text-xs font-black uppercase tracking-[.2em] text-white/45">Carrinho</div>
              <div className="mt-1 text-lg font-black">{quantity} {quantity === 1 ? 'item' : 'itens'} • {formatBRL(total)}</div>
            </div>
            <button disabled={!cart.length} onClick={() => { setCartOpen(false); setCheckoutOpen(true); }} className="inline-flex min-w-[260px] items-center justify-center gap-3 bg-[#d71920] px-7 py-5 text-base font-black uppercase transition hover:bg-white hover:text-black disabled:cursor-not-allowed disabled:opacity-40">
              Finalizar pedido <ArrowRight size={21} />
            </button>
          </div>
        </div>
      </section>
      ...
      {selectedProduct && (
        <div className="fixed inset-0 z-[55] flex items-center justify-center bg-black/80 p-4">
          <div className="w-full max-w-md border border-white/10 bg-[#0b0b0b] p-6 shadow-2xl">
            <div className="flex items-start justify-between gap-4">
              <div>
                <p className="text-xs font-black uppercase tracking-[.25em] text-[#d71920]">SEU HIT FOI SELECIONADO</p>
                <h2 className="rock-title mt-2 text-3xl">{selectedProduct.product_name}</h2>
              </div>
              <button onClick={() => setSelectedProduct(null)} className="border border-white/10 p-2 text-white/60 hover:bg-white hover:text-black" aria-label="Fechar"><X size={20} /></button>
            </div>
            <div className="mt-6 border-y border-white/10 py-5"><div className="flex items-center justify-between"><span className="text-white/50">Preço</span><strong className="text-2xl">{formatBRL(selectedProduct.product_price)}</strong></div></div>
            <div className="mt-6 grid gap-3">
              <button onClick={() => { add(selectedProduct, false); setSelectedProduct(null); }} className="w-full bg-[#d71920] px-5 py-4 font-black uppercase transition hover:bg-white hover:text-black">Adicionar</button>
              <button onClick={() => setSelectedProduct(null)} className="w-full border border-white/15 px-5 py-4 font-black uppercase transition hover:bg-white hover:text-black">Continuar comprando</button>
            </div>
          </div>
        </div>
      )}
