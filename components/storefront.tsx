  ...
  return (
    <main className="min-h-screen bg-[#050505] text-white">
      ...
      <section id="menu" className="mx-auto max-w-[1380px] px-5 py-16 lg:px-8">
        ...
        {!loading && products.length > 0 && <div className="quick-bar"><div><p>ATENDIMENTO RÁPIDO</p><h3 className="rock-title">UM CLIQUE. UM HIT.</h3></div><div className="quick-products">{products.slice(0, 12).map((product) => <button key={product.product_id} onClick={() => { add(product, false); setSelectedProduct(product); }}><Plus size={15} />{product.product_name}<b>{formatBRL(product.product_price)}</b></button>)}</div></div>}
        ...
        <div className="space-y-16">{Object.entries(grouped).map(([category, items]) => <section key={category}><div className="mb-6 flex items-center gap-4"><h3 className="rock-title text-3xl sm:text-4xl">{categoryLabels[category] ?? category}</h3><div className="h-[2px] flex-1 bg-[#d71920]" /></div><div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">{items.map((product) => <article key={product.product_id} className="product-card group"><div className="relative aspect-[4/3] overflow-hidden bg-black">{product.product_image_url ? <img src={product.product_image_url} alt={product.product_name} className="h-full w-full object-cover transition duration-500 group-hover:scale-105" /> : <div className="flex h-full items-center justify-center"><span className="rock-title text-5xl text-white/10">HRLG</span></div>}<div className="product-badge">★ {product.product_badge ?? 'Rock Hit'}</div></div><div className="p-5"><h4 className="rock-title text-2xl">{product.product_name}</h4><p className="mt-2 min-h-10 text-sm leading-6 text-white/55">{product.product_description ?? 'Preparado artesanalmente para o seu próximo show.'}</p><div className="mt-5 flex items-center justify-between gap-3"><strong className="text-xl">{formatBRL(product.product_price)}</strong><button onClick={() => {
  add(product, false);
  setSelectedProduct(product);
}}className="flex items-center gap-2 bg-[#d71920] px-4 py-3 text-xs font-black uppercase transition hover:bg-white hover:text-black"><Plus size={16} /> Adicionar</button></div></div></article>)}</div></section>)}</div>
      </section>
  ...
  <div className="fixed inset-0 z-[55] flex items-center justify-center bg-black/80 p-4">
    <div className="w-full max-w-md border border-white/10 bg-[#0b0b0b] p-6 shadow-2xl">
      <div className="flex items-start justify-between gap-4">
        <div>
          <p className="text-xs font-black uppercase tracking-[.25em] text-[#d71920]">
            SEU HIT FOI SELECIONADO
          </p>
          <h2 className="rock-title mt-2 text-3xl">
        ...
        <button
          ...
          className="border border-white/10 p-2 text-white/60 hover:bg-white hover:text-black"
          aria-label="Fechar"
        >
          <X size={20} />
        </button>
      ...
      <div className="mt-6 border-y border-white/10 py-5">
        <div className="flex items-center justify-between">
          ...
          <strong className="text-2xl">
            {formatBRL(selectedProduct.product_price)}
          </strong>
        </div>
      </div>
      ...
      <div className="mt-6 grid gap-3">
        <button
          ...
          className="w-full bg-[#d71920] px-5 py-4 font-black uppercase transition hover:bg-white hover:text-black"
        >
          Finalizar pedido
        </button>
        ...
        <button
          onClick={() => setSelectedProduct(null)}
          className="w-full border border-white/15 px-5 py-4 font-black uppercase transition hover:bg-white hover:text-black"
        >
          Continuar comprando
        </button>
      </div>
