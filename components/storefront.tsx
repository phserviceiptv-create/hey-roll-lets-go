        ...
        <div className="fixed inset-0 z-[55] flex items-center justify-center bg-black/80 p-4">
          <div className="w-full max-w-md border border-white/10 bg-[#0b0b0b] p-6 shadow-2xl">
            ...
            <div className="mt-6 grid gap-3">
              <button onClick={() => { add(selectedProduct, false); setSelectedProduct(null); setCheckoutOpen(true); }} className="w-full bg-[#d71920] px-5 py-4 font-black uppercase transition hover:bg-white hover:text-black">Finalizar pedido</button>
              <button onClick={() => setSelectedProduct(null)} className="w-full border border-white/15 px-5 py-4 font-black uppercase transition hover:bg-white hover:text-black">Voltar ao cardápio</button>
            </div>
          </div>
        </div>
      )}
