  ...
  return (
    <main className="min-h-screen bg-[#050505] text-white">
      ...
      <section id="menu" className="mx-auto max-w-[1380px] px-5 py-16 lg:px-8">
        ...
        <div className="space-y-16">{Object.entries(grouped).map(([category, items]) => <section key={category}><div className="mb-6 flex items-center gap-4"><h3 className="rock-title text-3xl sm:text-4xl">{categoryLabels[category] ?? category}</h3><div className="h-[2px] flex-1 bg-[#d71920]" /></div><div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">{items.map((product) => <article key={product.product_id} className="product-card group"><div className="relative aspect-[4/3] overflow-hidden bg-black">{product.product_image_url ? <img src={product.product_image_url} alt={product.product_name} className="h-full w-full object-cover transition duration-500 group-hover:scale-105" /> : <div className="flex h-full items-center justify-center"><span className="rock-title text-5xl text-white/10">HRLG</span></div>}<div className="product-badge">★ {product.product_badge ?? 'Rock Hit'}</div></div><div className="p-5"><h4 className="rock-title text-2xl">{product.product_name}</h4><p className="mt-2 min-h-10 text-sm leading-6 text-white/55">{product.product_description ?? 'Preparado artesanalmente para o seu próximo show.'}</p><div className="mt-5 flex items-center justify-between gap-3"><strong className="text-xl">{formatBRL(product.product_price)}</strong><button onClick={() => add(product, false)} className="flex items-center gap-2 bg-[#d71920] px-4 py-3 text-xs font-black uppercase transition hover:bg-white hover:text-black"><Plus size={16} /> Adicionar</button></div></div></article>)}</div></section>)}</div>
      </section>
